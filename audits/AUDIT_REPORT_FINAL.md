...
**Onchain token:** BORT (BAP578 NFA) `0x15b15df2fffF6653C21C11b93Fb8A7718Ce854Ce`
**Repository:** [`github.com/larpscantech/larpscan`](https://github.com/larpscantech/larpscan) (TypeScript MIT public)
**Audit window:** 2026-05-19 to 2026-05-21
...
|---|---|---|
| Smart contracts (onchain) | 🚨 **CRITICAL** | `BORT.createAgent()` is permissionless and bypasses the 0.01 BNB factory fee. Live PoC included. `PlatformConnectorLogic.handleAction()` is callable by anyone with any tokenId. Live PoC included. The ownership chain ends in a single externally owned account; no multisig no timelock. |
| Web / API (offchain) | 🚨 **CRITICAL** | `lib/wallet/signer.ts` (lines 262 to 409) actively **rewrites third party calldata** against `bnbshare.fun` to swap their vault factory and redirect 100 % of trading fees to a LarpScan controlled wallet. |
...
The product as built is **a centralized SaaS that signs real BSC transactions with a hot key modifies third party calldata in flight and ships LLM judged verdicts**. The onchain agents backing it are themselves a permissionless mint with pseudo random mock logic. Marketing language (trustless onchain decentralized) does not match implementation.
...
| Source of truth for contracts | Runtime bytecode + `eth_call` + `eth_estimateGas` + 4byte / openchain.xyz selector lookups. Solidity quotes are sourced from BscScan verified contract viewer for each address |
...
## 1. Onchain Findings (Smart Contracts)

### 1.1 Contract & Ownership Inventory

| # | Contract | Address | Onchain evidence |
|---|---|---|---|
| 1 | **BORT (BAP578 NFA)** | `0x15b15df2fffF6653C21C11b93Fb8A7718Ce854Ce` | `name()` returns BORT `symbol()` returns BORT `totalSupply()` returns `0x2b6c` (**11 116** NFTs minted) `proxiableUUID()` returns `0x360894…d12a0d5d` (canonical UUPS UUID) |
...
| 4 | **Owner of CircuitBreaker** (2nd hop) | `0x54e5651a7f62859cccB5b75613165a69c24eCb4e` | **Externally owned account (EOA).** `eth_getCode` returns `0x`. Nonce `0xe1b9` equals **57 785** transactions. Balance ≈ 0.0227 BNB. |
...
Compiler & license: solidity ^0.8.9 (per BscScan verified source). Source for each address is available in the BscScan verified contract viewer.
...
- `riskScore` is `keccak256(addr, block.timestamp) % 100`. Pure pseudo random and validator manipulable.
...
**Impact.** Any product feature (or third party integration) that consumes `SecurityAuditCompleted` `TradingActionCompleted` or `DAOActionCompleted` events as evidence is reading **fabricated data**. End users who pay 0.01 BNB to mint an NFA for AI driven onchain security auditing receive a random number generator.

**Severity:** **CRITICAL.** The primary product surface is non functional onchain.
...
- `external` with **no `onlyOwner` no `onlyAgentOwner` no `onlyRegistry` no `require(ownerOf(tokenId) == msg.sender)`**.
...
- **DoS / cost exhaustion** of off chain workers via spam events at roughly 28 k gas per call.
...
- The protocol economic model (agent creation pays for compute) is broken.
...
- Owner is `CircuitBreaker(0x907c46…)` whose owner is the EOA `0x54e5651a…` (57 785 nonces hot wallet).
...
| M3 | Low | `_addressToString` and `_uintToString` reimplemented locally. Dead code. Adds bytecode bloat |
...
| L1 | Info | `name` and `version` are mutable public state in `BasicAgentLogic` but immutable string literals in the other three. Inconsistent API across logic plugins |
| L2 | Info | No `IERC165.supportsInterface` exposed on the four logic contracts. UIs cannot introspect their interface |

---

### 1.11 Onchain risk summary
...
1. `POST /api/verify/orchestrate` is the entry point (Next.js App Router route handler).
2. Project metadata is resolved via `/api/project/discover` (BscScan / aggregators).
3. Website and X profile are scraped (`lib/scraper.ts` `lib/x-scraper.ts`).
...
7. WebM session recording is converted to MP4 (`ffmpeg-static`) and uploaded to Supabase Storage as evidence.
8. Verdict pipeline:
...
**Patch logic** in the `eth_sendTransaction` handler at lines **262 to 409** with the actual rebuild at **289 to 407** and replacement vaultData construction at **324 to 407**:
...
**Hardcoded test handles** at lines 331 to 339: `testuser` `larpscanbnb` `testuser2`..`testuser4` `lscantest01`..`lscantest05` `verifybot01`..`verifybot03` `agenttest01`..`agenttest03` `scantest_x1`..`scantest_x4` plus dynamic regex `qa\d{4}` patterns.
...
- The code comment explicitly describes this as bypassing the social signature check meaning circumventing the `bnbshare.fun` anti impersonation control.
- The cap `0x10 BNB` (≈ 100 000 000 000 000 000 wei) is a soft circuit breaker not a defense.

**Impact.**
- **For LarpScan.** Unilateral onchain modification of a third party protocol intended transaction. Operational legal and reputational risk. Even if framed as verification BNB Builders or `bnbshare.fun` would likely classify this as integrity tampering.
...
`lib/browser-agent/executor.ts` (roughly 1 850 LOC) injects a mock `window.ethereum` / EIP 6963 provider into the Playwright session so dApps see a connected wallet:
...
- A honeypot or malicious contract can drain the investigation wallet via `eth_sendTransaction` (no calldata level filter beyond P1 specific bnbshare patch).
- Bypasses the wallet UX level prompt that users expect.
...
- Compromise of any of {Vercel project Supabase service role Browserless token an AI coauthored commit that leaks env any team member GitHub session} leads to wallet drain.
- Wallet is on **BSC mainnet** so real funds are at risk.
...
**No escaping. No content fencing. No instruction injection mitigation.** An adversary that controls the page being scraped (or owns an NFA whose system_prompt feeds into the verdict pipeline) can inject text such as:
...
- Body field `forceReverify=true` (line 86) closes any active runs in `['pending' 'verifying' 'extracting' 'analyzing' 'queued']` by updating their status to `'failed'` (lines 92 to 103).

**Direct attack chain.**
1. Attacker spams `POST /api/verify/orchestrate` with `forceReverify=true` for an active competitor `projectId`.
...
| ID | Severity | Finding |
|---|---|---|
| M1 | Medium | `lib/wagmi-config.ts` falls back to WalletConnect `projectId = 'demo-larpscan'` if `NEXT_PUBLIC_WC_PROJECT_ID` is unset. WC projectIds are public so this is not a secret leak but it indicates incomplete prod env management |
...
| M3 | Medium High | The verdict pipeline LLM fallback (Layer 2) means the final verdict for inconclusive cases is **non deterministic**. GPT 4.1 + temperature 0 helps but does not eliminate drift over time and per NFA system_prompts diverge results across agents |
...
| M5 | Low | `lib/browser-agent/executor.ts` `ALWAYS_BLOCKED_PATTERNS` filters only UI text not transaction calldata. Does **not** protect against the P1 rewrite or against a malicious dApp transaction |
...
| L3 | Info | Wayback Machine has no snapshots of `larpscan.sh`. The site is brand new |
...
| L6 | Info | `/api/verify/orchestrate` `maxDuration = 120 s`. `verifier.ts` 450 to 510 s hard timeouts. Long running Playwright jobs on Vercel Lambdas are expensive and prone to half state |
| L7 | Info | The `Reusing completed run` cache key is `(project_id status='complete')` only. Once a verdict is recorded drift after the project changes is masked unless `forceReverify` is set |
...
| NFA agent system | BAP578 NFTs at `0x15b15df…` + per NFT system_prompt injected into verdict LLM | ✅ but tainted by P5 |
| Onchain agents act onchain | Logic contracts are random pseudo mocks (C1) | ❌ |
...
All values captured **2026-05-21 around 19:46 to 19:48 UTC** via Google Public DNS (8.8.8.8) and `https://bsc-dataseed.binance.org/`. Vercel anycast IPs rotate by region/POP. The IPs below are valid for `fra1` (Frankfurt) at scrape time.
...
`curl --resolve www.larpscan.sh:443:<any-Vercel-IP> https://www.larpscan.sh/` returns HTTP/2 200 same content `x-vercel-cache: HIT`. This proves there is **no real IP hidden behind a CDN to reveal**. Vercel **is** the origin and traffic reaches the edge whichever pool IP you target. Classical origin reveal does not apply.
...
- Registered **2026-03-25** (roughly 2 months at audit date).
...
| Onchain logic functionality | 🚨 CRITICAL | C1: pseudo random mocks (BscScan verified source) |
...
13. **Decouple marketing language** (trustless onchain verification) from architecture. The current product is centralized AI + hot wallet + Supabase. Presenting it as decentralized is itself a LARP signal.

---

## 6. Final verdicts

- **Onchain (BORT + logic + treasury):** 🚨 **NOT FIT FOR FINANCIAL USE.** Permissionless mint (C3) and event spoofing (C2) are proven on mainnet via `eth_call`. Random mock logic (C1) single EOA control (H1) and any recipient `emergencyWithdraw` (C4) compound the risk.
...
The gap between the marketing surface (AI powered onchain NFA driven decentralized) and the implementation (centralized SaaS + hot wallet + LLM judge + permissionless mock NFTs) combined with the calldata rewrite patch is the main risk for users and integrators.
