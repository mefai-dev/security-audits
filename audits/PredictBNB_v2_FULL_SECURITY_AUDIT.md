...
## Section 11: Team Questions - Direct Answers

### Q: What parameters need to be adjusted for operator-controlled resolution?

**Answer:** The operator-controlled win/loss outcome model is acceptable, but requires these specific adjustments:

| Parameter | Current | Required Change | Why |
|-----------|---------|-----------------|-----|
| Owner positions (Lines 110-112) | Owner gets YES+NO | **DELETE completely** | Prevents owner from profiting unfairly |
| Resolution timing | Instant | **Add 24h timelock** | Allows users to verify/dispute |
| Betting cutoff | None | **Add 5 min before endTime** | Prevents last-second manipulation |
| Minimum bet | 0 (any amount) | **Set 100 tokens minimum** | Prevents dust/griefing attacks |
| Max market duration | Unlimited | **Set 90 days maximum** | Prevents indefinite fund locking |
| Pause mechanism | None | **Add Pausable** | Emergency stop capability |

### Q: Are there any issues with using our own token (p4BNB) instead of BNB/USDT?

**Answer:** No issues. The current implementation is correct for custom ERC20 tokens:

| Component | Status | Notes |
|-----------|--------|-------|
| `IERC20` interface | CORRECT | Properly imported |
| `SafeERC20` library | CORRECT | Using `safeTransferFrom` and `safeTransfer` |
| Token address | CORRECT | Set as `immutable` in constructor |
| Decimal handling | CORRECT | Uses `ether` unit (10^18) |
| Approval flow | CORRECT | Users must approve contract before betting |

**No changes needed for token integration.**

### Q: Can we keep operator-controlled resolution?

**Answer:** Yes, but ONLY if you implement these mandatory changes:

```
MANDATORY:
1. Remove owner balance assignments (Lines 110-112)
2. Add 24-hour resolution timelock
3. Add betting cutoff (5 min before endTime)

RECOMMENDED:
4. Add minimum bet (100 tokens)
5. Add Pausable
6. Make ownership transferable
7. Add maximum market duration (90 days)
```

### Q: What is the risk level after implementing fixes?

| State | Risk Score | Acceptable? |
|-------|------------|-------------|
| Current (no fixes) | 35/100 | NO - Do not deploy |
| After mandatory fixes | 55/100 | Acceptable with caution |
| After all fixes | 65/100 | Acceptable for production |
| With oracle integration | 80/100 | Recommended for trustless operation |

---

## Section 12: Technical Summary for Development Team
...
## Section 13: Risk Assessment After Fixes
